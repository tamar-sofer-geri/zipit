...
function renderItemRow(item, r){
  ...
  if (item.note) main.appendChild(el('span', { class: 'item-note', title: item.note, text: 'ⓘ' }));
  ...
  if (r.adjustments.length){
    var text = r.adjustments.map(function(a){ return a.text; }).join(' ');
    var title = r.adjustments.map(function(a){ return a.rule.label + ': ' + a.rule.note; }).join('\n');
    row.appendChild(el('span', { class: 'badge', title: title, text: text }));
  }
  return row;
}
...
function boot(){
  ...
  root.innerHTML =
    '<header class="topbar">' +
      '<div class="brand"><span class="brand-mark">🧳</span><div><h1>Manifest</h1><p class="eyebrow">packing, calculated</p></div></div>' +
